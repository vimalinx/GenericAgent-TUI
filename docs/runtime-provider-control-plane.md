...
OMP main-runtime context packs use a GA-TUI permission profile. The default is
...
The isolated OMP config generated by GA-TUI defaults `tools.approvalMode` to
`write` rather than `always-ask`. Read/write-tier OMP tools can run headless in
the GA-TUI-owned runtime directory, while exec-tier approval prompts are bridged
through the active runtime request permission profile. `permission_profile:full`
may auto-approve non-risky exec prompts; prompts that look like deletion,
