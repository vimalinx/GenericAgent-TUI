...
- OMP host tool result followed by no `agent_end` or visible assistant text -> host-tool follow-up watchdog emits a bounded `[Oh My Pi]` fallback `done` item, clears `is_running`, decrements unfinished task count, and does not emit a memory candidate.
...
- Tests must assert a stalled Shuheng host-tool follow-up exits running state through the host-tool watchdog fallback without creating an Oh My Pi memory candidate.
