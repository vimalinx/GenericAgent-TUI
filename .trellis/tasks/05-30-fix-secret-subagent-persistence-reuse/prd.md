...
* Main-agent hidden `<ga-tui>` subagent controls are also supported in Secret mode, but only dedicated Secret subagent controls execute.
...
* Secret prompt guidance must tell the main agent that persistent Secret subagents use the same `<ga-tui>{"action":"subagent_create","persistent":true,...}</ga-tui>` control, but are stored in the encrypted Secret vault.
