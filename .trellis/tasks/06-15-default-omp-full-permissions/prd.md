...
Make the OMP-backed GA-TUI main runtime usable as a real agent by default instead of presenting itself as a read-only `specialist`. The default OMP main context pack should advertise a full, practical tool capability profile while preserving GA-TUI as the owner of approvals, long-term memory, task ledgers, artifacts, and isolated OMP runtime files.
...
* OMP is the default runtime provider on this branch, but GA-TUI remains the policy and memory governance owner.
* OMP runtime files are already projected into a GA-TUI-owned isolated agent directory, not the user's system `~/.omp/agent`.
...
* `full` profile must not remove GA-TUI approval gates for high-risk actions such as deploy, external send, deletion, spending money, secret access, and permission-policy changes.
...
Decision: Make permission scope a GA-TUI-owned context-pack profile. Default only the OMP main runtime to `full`; keep governed policy gates and candidate-only memory intact.
...
* Letting OMP bypass GA-TUI approvals for high-risk actions.
* Letting OMP write long-term memory, schedules, ledgers, or artifacts directly outside GA-TUI-owned APIs.
* Building a full TUI settings screen for permission profiles in this task.

## Technical Notes

* Relevant code: `src/ga_tui/app.py`, `src/ga_tui/ohmypi_provider.py`, `scripts/check_policy_gates.py`.
